...
"""
This module holds the Duplicates class which scales the number of duplicates
...
class Duplicates(Scenario):
    def __init__(self, main_directory: str, verbose: bool,
        ...
        ----------
        main_directory : str
            Root directory for generating instances of Duplicates.
        verbose : bool
            Verbose logging enabled or not.
        percentage : float
            Percentage duplicates to generate, for example 50% results into
    ...
    def path(self) -> str:
        ...
        -------
        path : str
            File path for the Duplicates' instance.
        """
        key = f'duplicates_{self._percentage}_percentage'
    ...
    def _update_dataframe(self, dataframe: DataFrame):
        ...
        for i in list(sample.index):
            for j in range(1, self._number_of_properties + 1):
                dataframe.loc[i, f'p{j}'] = 'DUPLICATE'
            dataframe.loc[i, 'id'] = numpy.iinfo(numpy.int64).max

        return dataframe

    def _generate_mapping(self) -> Graph:
        """Generate a [R2]RML mapping for a Duplicates instance.
    ...
    def _generate_scenario(self) -> bool:
        ...
        success : bool
            True if successfull, false otherwise
        """
        name: str = f'duplicates_{self._percentage}'
        description: str = f'Duplicates {self._percentage}'
        iri: str = f'http://example.org/duplicates/{self._percentage}/'
